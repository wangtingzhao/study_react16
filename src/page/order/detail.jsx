        ...
        <div className="form-group">
          ...
          <div className="col-md-5">
            <p className='form-control-static'>
              ...
              {
                this.state.orderInfo.status === 200 
                ? <button className='btn btn-default btn-sm btn-send-goods'
                  onClick={(e) => this.onSeahGoods()}></button>
                : null
              }
            </p>
          </div>
          ...
          <TableList tableHeads={tableHeads}>
            {
              productList.map((product, index) => {
                return (
                    <tr key={index}>
                      <th> <img className='p-img' src={`${this.state.orderInfo.imageHost}${product.productImage}`} alt={product.productName}/></th>
                      <th>{product.productName}</th>
                      <th>￥{product.currentUnitPrice}</th>
